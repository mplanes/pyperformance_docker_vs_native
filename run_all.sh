...
python3.7 -m pyperformance run -o $native_results
...
docker run -v /tmp:/tmp -v /tmp/venv_docker_debian:/app/venv pyperfdebian pyperformance run --python=python3 -o $docker_results
