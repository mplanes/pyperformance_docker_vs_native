...
docker run -v /tmp:/tmp -v /tmp/venv_docker:/venv pyperfdebian pyperformance run --python=python3 -b nqueens -o $docker_results

docker build . -f Dockerfile.py3perfdebian2 -t pyperfdebian2
docker run -v /tmp:/tmp -v /tmp/venv_docker:/venv  pyperfdebian2 pyperformance run --python=python3 -b nqueens -o $docker_results2
